...
angular.module('liskApp').controller('sendTransactionController', ['oxyAPI', '$scope', 'sendTransactionModal', '$http', 'userService', 'feeService', '$timeout', '$filter', function (oxyAPI, $scope, sendTransactionModal, $http, userService, feeService, $timeout, $filter) {
    ...
    if ($scope.to.trim() === '') {
      ...
      if (correctAddress) {
        if ($scope.isCorrectValue($scope.amount)) {
          if ($scope.note && $scope.note.length > 64) {
            $scope.errorMessage.note = 'The length of note field must be less than 64 character';
            $scope.presendError = true;
          } else {
            return onValid();
          }
  ...
  $scope.isCorrectValue = function (currency, throwError) {
    ...
    if (amount == '') {
      return error('LWF amount can not be blank');
    }
    ...
    if (parts.length == 1) {
      ...
      if (parts[1].length > 8) {
        return error('LWF amount must not have more than 8 decimal places');
      ...
      return error('LWF amount must have only one decimal point');
    }
    ...
    if (amount == '0' && fraction == '00000000') {
      return error('LWF amount can not be zero');
    }
    ...
    if (!/^\d+$/.test(result)) {
      return error('LWF amount contains non-numeric characters');
    }
  ...
  $scope.convertLWF = function (currency) {
  ...
  $scope.sendTransaction = function (secretPhrase, withSecond) {
    ...
    var data = {
      secret: secretPhrase,
      amount: $scope.convertLWF($scope.amount),
    ...
    if (!$scope.sending) {
      ...
      var transaction = shiftjs.transaction.createTransaction(data.recipientId, data.amount, data.secret, data.secondSecret, data.note);
